...
must_haves:
  truths:
    ...
    - "Redirect resolution follows redirects and returns final URL, returns None on timeout/error"
  ...
  artifacts:
    - path: "tests/test_slack/test_urls.py"
      provides: "URL extraction, user note, and redirect resolution tests"
      min_lines: 100
  ...
  key_links:
    - from: "tests/test_slack/test_urls.py"
      ...
      via: "imports extract_urls, extract_user_note, resolve_url, resolve_urls"
...
All `extract_urls` and `extract_user_note` tests are plain `def test_*()` functions (sync, pure functions). Follow project convention: `test_{function}_{scenario}` naming.

3. Add async tests for `resolve_url` and `resolve_urls` to the SAME file (INGEST-08):

These functions are async and use `httpx.AsyncClient`, so tests need `pytest.mark.asyncio` (or rely on project `asyncio_mode = "auto"`). Mock `httpx.AsyncClient` using `unittest.mock.patch` or `respx` (if available) or `pytest-httpx`.

**resolve_url tests:**
- `test_resolve_url_follows_redirect` -- mock httpx to return a response with a different final URL (e.g., request `https://t.co/abc` -> response.url is `https://example.com/article`). Assert returns `"https://example.com/article"`.
- `test_resolve_url_timeout_returns_none` -- mock httpx to raise `httpx.TimeoutException`. Assert returns `None`.
- `test_resolve_url_too_many_redirects_returns_none` -- mock httpx to raise `httpx.TooManyRedirects`. Assert returns `None`.
- `test_resolve_url_http_error_returns_none` -- mock httpx to raise `httpx.HTTPError`. Assert returns `None`.

**resolve_urls tests:**
- `test_resolve_urls_returns_successful_only` -- pass 3 URLs; mock so URL 1 resolves, URL 2 times out (None), URL 3 resolves. Assert result is `[resolved_1, resolved_3]` (failures filtered out).
- `test_resolve_urls_skips_failures` -- pass 2 URLs; mock so both fail. Assert result is `[]`.
- `test_resolve_urls_empty_input` -- pass empty list. Assert result is `[]`.
  </action>
  <verify>
- `uv run pytest tests/test_slack/test_urls.py -v` -- all tests pass
- `uv run ruff check tests/test_slack/test_urls.py` -- linter clean
  </verify>
  <done>All URL extraction, user note, and redirect resolution tests pass. Both `<url>` and `<url|label>` formats correctly handled. User mentions, channel refs, and special commands excluded. User note correctly strips URL markup. resolve_url follows redirects and returns None on timeout/error. resolve_urls filters out failures.</done>
</task>
...
4. Each INGEST requirement has at least one test verifying it:
   ...
   - INGEST-08: test_resolve_url_follows_redirect, test_resolve_url_timeout_returns_none, test_resolve_url_too_many_redirects_returns_none, test_resolve_urls_skips_failures
</verification>
...
- Redirect resolution tests cover: successful redirect follow, timeout returns None, too many redirects returns None, resolve_urls filters failures
